...
```
설치 (한 번만, 15~25분)  →  start.command 더블클릭  →  브라우저 4 단계  →  PDF 다운로드
```
...
claude
```
→ Claude Code 가 처음 켜지면서 **브라우저가 자동으로 열림** → Claude 계정으로 로그인 → "Authorize" → "터미널로 돌아가기" 같은 안내. 터미널로 다시 가서:
...
> ❗ Claude **Pro ($20/월)** 또는 **Max ($100/월)** 구독이 있어야 해요. [claude.ai](https://claude.ai/) 에서 먼저 가입.
>
> 💡 **ChatGPT Plus/Team 사용자**라면 Claude Code 대신 **Codex** 도 OK. `claude` 자리에 `codex` 로 바꿔서 같은 흐름 (`npm install -g @openai/codex` → `codex login`). 둘 중 한 도구만 있으면 돼요.

> 💡 **slides-grab 기술 설치** 단계는 이전엔 사용자가 직접 paste 해야 했는데, **우리 앱이 자동으로** 해 줘서 빠졌어요. 화면 1 에서 [⚡ 자동 설치] 버튼 한 번만 클릭하면 끝 (Claude 1~3분 / Codex 10초).
...
폴더 안의 **`start.command`** 더블클릭.

**"신뢰할 수 없는 개발자" 경고가 뜨면**:
- **마우스 오른쪽 → 열기 → "열기"** (한 번만 이렇게)
- 다음부터는 그냥 더블클릭으로 OK
...
이런 화면이 보임 (Claude Code 와 Codex 두 카드):

```
Claude Code                       Codex
✓ 설치됨 · 버전 2.x.x             ✓ 설치됨 · 버전 0.x.x
✗ slides-grab 기술 안 됨          ✗ slides-grab 기술 안 됨
   [⚡ 자동 설치]                    [⚡ 자동 설치]
```

**둘 중 한 카드의 ⚡ 자동 설치 클릭** → 자동으로:
...
- Claude Code: 1~3분 (Claude 가 fetch + 파일 쓰기)
- Codex: 10초 정도 (`npx skills add` 한 줄 copy)
...
1. `bitreespark_II` 폴더의 **`start.command`** 더블클릭 (5초)
2. 브라우저 자동 open
...
| 터미널에서 `command not found: pnpm` | Node.js 설치 후 터미널 새 창 열기. 그래도 안 되면 1-① 다시 |
| `command not found: claude` | `npm install -g @anthropic-ai/claude-code` 안 함 |
| 화면 1 에서 "Claude Code 설치 안 됨" | 터미널에서 `claude --version` 직접 확인. 안 나오면 1-② 다시 |
...
| ⚡ 자동 설치가 3분 넘게 안 끝남 | 한 번 더 클릭 또는 터미널에서 직접 `claude` 후 안내된 줄 paste |
...
| 시점 | 소요 |
|---|---|
| Node.js + pnpm + Claude Code 설치 (한 번만) | 10~20분 |
| 첫 실행 (`start.command` 첫 클릭) | 5~10분 (의존성 + Chromium ~150MB) |
| 화면 1 의 slides-grab 기술 자동 설치 (한 번만) | 1~3분 |
| **여기까지 = 처음 한 번만**, 합치면 | **20~35분** |
| 매일 켜기 | 5초 |
...
├── providers/      # claude-code / codex 추상화 + skill 점검
