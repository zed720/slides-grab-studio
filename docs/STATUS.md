...
- 2026-05-25: 내 양식 v2 minimal — archetype 갤러리 (표지·본문 × 2안). v1 의 brand kit 위에 사용자가 archetype 별로 모양 픽 → generator prompt 에 그 archetype HTML 예시 박힘 → AI 가 새 콘텐츠로 같은 layout 만듦. 4 옵션: COVER_A "단정한 좌측" / COVER_B "풀 컬러 강조" / BODY_A "좌 헤딩+우 본문" / BODY_B "불릿 위주". BITREE 패턴 재확장 — `brandKitToInstruction()` 이 archetypes 받으면 `### 표지 슬라이드 ###` `### 본문 슬라이드 ###` 섹션 추가, 각각 옵션 sample HTML 박음 (strong anchor). preview API 도 cover archetype 받아 즉시 반영 (BR placeholder span → 로고 <img> 후처리 교체). 새 파일: `lib/custom-templates/archetypes.ts` (4 옵션 + ARCHETYPES 카탈로그 + getArchetypeOption), `components/ArchetypePicker.tsx`. 기존 brand_json 안 nested key 추가만이라 DB migration 불필요.
- 2026-05-25: 내 양식 v2 — 미리보기 iframe 4곳을 `ScaledSlideFrame` 으로 통일. archetype 옵션 카드의 작은 iframe (220px) 에 fixed px 폰트 (48px h1 등) 가 그대로 들어가 글자가 폭발/스크롤되던 문제. iframe viewport 960×540 으로 두고 transform:scale 로 카드에 맞추는 패턴 (slides-grab 카드 디자인이 이미 쓰던 것) 으로 통일.
- 2026-05-25: fix — `plannedSlideTotal` 의 fallback 6 이 `Math.max()` 인자에 박혀 있어 outline 이 5장이면 6 으로 강제 올림되던 버그. UI 가 "5/6장 + 6번째 만드는 중..." 가짜 placeholder 표시. 수정: max 가 0 일 때만 fallback 6 적용.
- 2026-05-26: 내 양식 v2b — 나머지 5 archetype 추가 (목차/표/차트/이미지/마무리 × 2안 = 10 옵션 추가, 합계 14 옵션). `archetypes.ts` 데이터만 추가 — `ArchetypePicker` UI 는 `ARCHETYPES` 배열을 iterate 하므로 자동으로 7 종 다 표시 (UI 코드 변경 0). `ArchetypeKey` 7키 union, `instruction.ts` 의 archLabels 와 PATCH/preview API 의 검증 loop 도 `ARCH_KEYS` 배열로 generic 화.
  - 새 옵션 디자인: TOC_A "번호+줄" / TOC_B "사이드 강조" (좌 주색 패널 + 우 항목), TABLE_A "일반 표" (주색 헤더 + zebra) / TABLE_B "카드 형" (강조 bar), CHART_A "막대 차트" / CHART_B "큰 숫자 강조" (주색 배경 + 거대 강조색 숫자), IMAGE_A "좌 사진 + 우 텍스트" / IMAGE_B "풀 사진 + 오버레이", CLOSING_A "한 줄 굵게" (주색 배경 + 강조색 키워드) / CLOSING_B "단정한 메시지" (흰 배경 + 라벨 + rule line). 모든 디자인이 사용자 brand 의 primaryColor/accentColor 즉시 반영.
- 2026-05-26: 내 양식 — 새 양식 만들기 시 이름 먼저 받기 (무방비 생성 방지). 기존엔 + 버튼 클릭 즉시 POST + "새 양식" default 이름 + redirect → 잘못 클릭만 해도 빈 양식 row 영구 생성. 수정: 버튼 클릭 → 버튼 자리에 inline input + 취소/만들기 토글, Enter submit / Esc cancel. `/my-templates/new` 직접 진입도 자동 POST 대신 `/my-templates` redirect 로 통일.
- 2026-05-26: 내 양식 v3 — AI 새 모양 만들기 (🪄). 사용자가 archetype 별로 자연어 + 자주 쓰는 칩으로 새 옵션 요청 → claude/codex spawn → HTML 생성 → brand_json 의 `aiArchetypes` 에 영구 저장 → 갤러리에 정적 옵션과 함께 표시 (🪄 AI 배지). 7 종 archetype 모두 지원.
  - **타입**: `AIArchetypeEntry { id (ai-<ts>-<rand>), archKey, name, description, html, createdAt }`, `BrandKit.aiArchetypes: AIArchetypesByKey`. html 은 brand 적용된 final snapshot — brand 색 바꿔도 그대로 유지 (사용자가 새로 만들거나 삭제).
  - **API**: `POST /api/my-templates/[id]/archetypes/generate` (검증, 동시 1 generation in-memory lock, 호출 후 `addAIArchetype` 영구 저장), `DELETE /api/my-templates/[id]/archetypes/[archKey]/[entryId]` (AI 옵션 개별 삭제, ai- prefix 만 허용).
  - **spawn 모듈** `lib/custom-templates/ai-generation.ts`: `customTemplatesDir/<id>/generations/<jobId>/` tmp 디렉토리에 claude/codex spawn → AI 가 `output-1.html`…`output-N.html` 작성하도록 prompt (archetype 별 sample text 안내 + brand 명시 + logo 절대 경로). 결과 검증 — `<script>` 태그 제거, html/body 구조 확인, size 한도. cleanup — tmp 디렉토리 삭제 (영구 저장은 html string 뿐). 4분 타임아웃, ENOENT 시 친절 안내.
  - **통합 lookup** `resolveArchetype(key, id, brand)`: 정적 옵션 + AI 옵션 둘 다 검색. `isStatic` 플래그로 호출자 분기. instruction.ts / preview.ts / PATCH route 모두 사용 — AI id (ai-*) 도 valid 로 인정, generator prompt 에도 AI 옵션 sample HTML 박힘.
  - **UI**: archetype 헤더에 🪄 새 모양 만들기 버튼 + 클릭 시 인라인 패널 토글 (한 번에 하나만 열림). 패널 내용 = 자연어 textarea + 자주 쓰는 칩 5개 (🏢 우리 브랜드 적용 default) + 개수 (1/2/3) + 예상 시간 + 토큰 안내 + 🪄 만들기. AI 옵션 카드는 정적 옵션 앞에 배치 (사용자 직접 만든 거 우선), 🪄 AI 배지 + hover 시 우하단 ✕ 삭제.
  - mockup `docs/mockups/14-template-builder.html` 의 차트 archetype 펼친 패널 모습대로. 7 종 다 동시 지원 (minimal scope 였지만 코드 변경 같아 모두 풀어줌).
