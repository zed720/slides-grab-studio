...
- 2026-05-26: 내 양식 v2b — 나머지 5 archetype 추가 (목차/표/차트/이미지/마무리 × 2안 = 10 옵션 추가, 합계 14 옵션). `archetypes.ts` 데이터만 추가 — `ArchetypePicker` UI 는 `ARCHETYPES` 배열을 iterate 하므로 자동으로 7 종 다 표시 (UI 코드 변경 0). `ArchetypeKey` 7키 union, `instruction.ts` 의 archLabels 와 PATCH/preview API 의 검증 loop 도 `ARCH_KEYS` 배열로 generic 화. 이제 v3 (AI 새 모양 만들기) 만 남음.
